...
struct block *findFreeBlock(struct block **last, size_t size) 
{
   ...
   *last = curr;
   ...
   if (curr == NULL){
     curr = FreeList;
     while (curr!= original && !(curr->free && curr->size >= size)) 
     {
       *last = curr;
       curr = curr->next;
     }
...
void *malloc(size_t size) 
{
  ...
  {
    //crete new freeblock base on the extra free block of next
    size_t b_size = (next->size)- size;
    struct block *b_block = next->next;
    struct block freeBlock = { b_size , b_block, true};
